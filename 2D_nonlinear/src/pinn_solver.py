"""
PINN solver 2D non-lineaire — formulation forte via flux AD.

EDP :  -div(k(u)*grad(u)) = f(x,y),  k(u) = 1 + beta*u^2

Le residu PDE est calcule via AD sur le flux :
    flux = k(u) * grad(u)
    residual = -div(flux) - f
...
from exact_solution import (
    ALPHA, G0, DTYPE, BETA_DEFAULT,
    f_source_torch, u_exact_np, u_exact_dx_np, u_exact_dy_np,
)
from utils import (
    gauss_legendre_torch, grad_net, div_flux,
    compute_errors_2d, DEVICE,
)
...
def _make_points(cfg):
    ...
    xy_f = torch.stack([xx.flatten(), yy.flatten()], dim=1)
...
def _loss_pinn(net, xy_f, xy_right, xy_bottom, xy_top,
    ...
    # 1. Residu PDE via flux AD : -div(k(u)*grad(u)) = f
    u = net(xy_f)
    u_x, u_y = grad_net(u, xy_f)
    k_u = 1.0 + beta * u ** 2
    flux_x = k_u * u_x
    flux_y = k_u * u_y
    div_f = div_flux(flux_x, flux_y, xy_f)
    r_f = -div_f - f_source_torch(xy_f, beta=beta)
    ...
    xy_quad = torch.stack([x_rep, y_rep], dim=1)
    u_quad = net(xy_quad).reshape(n_y, n_qx)
    integrals = torch.sum(w_quad.squeeze() * u_quad, dim=1, keepdim=True)
...
def train_pinn(config=None, verbose=True):
    ...
    if verbose:
        ...
        nf = cfg["n_f_per_dim"]
        print(f"  PINN 2D NL | {cfg['n_hidden']}x{cfg['n_layers']} ({n_params} params) | "
              f"N_f={nf}x{nf}={nf**2} | beta={beta} | seed={cfg['seed']}")
